...
namespace
{
// minizip >= 1.3 (zlib 1.3) replaced the integer case-sensitivity flag with a
// filename comparer callback. The comparer below matches legacy flag value 2
// (native, case-sensitive string comparison).
#ifndef MINIZIP_INT_LOCATE
    int pmpCaseSensitiveUnzComparer(unzFile, const char* a, const char* b)
    {
    ...
    {
#ifdef MINIZIP_INT_LOCATE
        if (unzLocateFile(uf, filename.toLatin1(), 2) != UNZ_OK) {
#else
        if (unzLocateFile(uf, filename.toLatin1(), pmpCaseSensitiveUnzComparer) != UNZ_OK) {
#endif
